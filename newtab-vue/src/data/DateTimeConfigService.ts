import { isDateStyleType, isTimeStyleType, type DateStyleType, type Hour12Type, type TimeStyleType } from "@/types/DateTimeFormatOptionTypes";
...
export default class DateTimeConfigService {
    ...
    constructor() {
        ...
        if (isTimeStyleType(storageTimeStyle))
            this.timeStyle.value = storageTimeStyle as TimeStyleType;
    ...
    private _timeStyleWatcher(newTimeStyle: TimeStyleType, oldTimeStyle: TimeStyleType) {
        localStorage.setItem('time-style', newTimeStyle ?? '--undefined--');
    }

    private _dateStyleWatcher(newDateStyle: DateStyleType, oldDateStyle: DateStyleType) {
        localStorage.setItem('date-style', newDateStyle ?? '--undefined--');
    }

    private _hour12Watcher(newHour12: Hour12Type, oldHour12: Hour12Type) {
